...
RUNTIME_RESULTS_DIR = compat.runtime_dir() / "results"
...
def start_auto_guard() -> None:
    if compat.IS_WINDOWS:
        log("Windows detected; Auto Guard persistence is managed by guard_panel_server.py / Task Scheduler.")
        return
...
def install_ollama_if_needed() -> str:
    ...
    if ollama:
        ...
        return ollama
    if compat.IS_WINDOWS:
        raise RuntimeError("Automatic Ollama installation is not supported on Windows.")
