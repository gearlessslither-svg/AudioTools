...
PANEL_TOKEN_PATH = RESULTS_DIR / "panel_api_token.txt"
LAUNCH_RUNTIME_DIR = compat.runtime_dir()
...
LAUNCH_AUTO_GUARD_LOG_PATH = LAUNCH_RUNTIME_DIR / "results" / "auto_guard_supervisor.log"
LAUNCH_AUTO_GUARD_LABEL = (
    compat.WINDOWS_AUTO_GUARD_TASK_NAME
    if compat.IS_WINDOWS
    else "com.story.mullvad-speed-guard.auto-guard"
)
LAUNCH_AUTO_GUARD_PLIST = (
    LAUNCH_RUNTIME_DIR / f"{LAUNCH_AUTO_GUARD_LABEL}.plist"
    if compat.IS_WINDOWS
    else Path.home() / "Library" / "LaunchAgents" / f"{LAUNCH_AUTO_GUARD_LABEL}.plist"
)
WINDOWS_AUTO_GUARD_CMD = LAUNCH_RUNTIME_DIR / "run_auto_guard.cmd"
PYTHON = compat.python_executable()
APP_PATH = compat.default_path()
DEFAULT_PORT = 18790
PANEL_VERSION = "2026-06-18-windows-compatible"
...
def load_panel_api_token() -> str:
    configured = os.environ.get("MSG_PANEL_API_TOKEN", "").strip()
    if configured:
        return configured
    try:
        token = PANEL_TOKEN_PATH.read_text(encoding="utf-8").strip()
        if len(token) >= 24:
            return token
    except Exception:
        pass
    ...
    PANEL_TOKEN_PATH.write_text(token + "\n", encoding="utf-8")
    try:
...
PANEL_API_TOKEN = load_panel_api_token()
...
<body>
  ...
  <main>
    <section id="guardBanner" class="guard-banner paused">
      <div class="metric">
        ...
        <div id="guardHint" class="hint">Checking supervisor.</div>
      </div>
      <div class="metric">
        <div class="label">Supervisor</div>
        <div id="guardSupervisor" class="value compact">--</div>
        <div id="guardSupervisorHint" class="hint">A supervisor is required for unattended recovery.</div>
      </div>
      <div class="metric">
    ...
    <section>
      ...
      <div class="grid">
        ...
        <div>
          ...
          <input id="speedEvery" type="number" min="0" step="1" value="0">
        </div>
        <div>
        ...
        <div>
          <label for="allowActiveSwitching">Allow active switching</label>
          <input id="allowActiveSwitching" type="checkbox">
        </div>
      </div>
      <div class="actions">
  ...
  <script>
    const ids = [
      ...
      "urlChecks", "refreshPoolOnFailure", "disableIdleRefresh", "allowActiveSwitching"
    ];
    ...
    function settings() {
      return {
        ...
        allow_active_switching: el.allowActiveSwitching.checked,
    ...
    function renderAutoGuard(inventory) {
      ...
      guardBanner.className = "guard-banner " + (
        actionPaused ? "warning" : (mode === "launchagent" || mode === "scheduled-task") ? "running" : running ? "warning" : "paused"
      );
      guardState.textContent = actionPaused ? "PAUSED" : running ? "RUNNING" : "PAUSED";
      guardSupervisor.textContent = mode === "launchagent"
        ? "LaunchAgent"
        : mode === "scheduled-task"
          ? "Task Scheduler"
          : running
      ...
      if (actionPaused) {
        ...
        guardSupervisorHint.textContent = (mode === "launchagent" || mode === "scheduled-task") ? (status.launch_state || "running") : "not supervised";
        guardIntentHint.textContent = "The process stays alive and resumes when the task exits or the lock expires.";
      } else if (mode === "launchagent" || mode === "scheduled-task") {
        guardHint.textContent = mode === "scheduled-task"
          ? "Background recovery is registered with Windows Task Scheduler."
          : "Background recovery is supervised by macOS.";
        ...
        guardHint.textContent = "Running, but not under a persistent supervisor. Press Start Auto Guard to register it.";
        guardSupervisorHint.textContent = "This mode will not auto-restart after a crash.";
        guardIntentHint.textContent = "Protection is partial until the supervisor takes over.";
      } else {
        guardHint.textContent = "Paused by user or not started.";
        guardSupervisorHint.textContent = status.error || "No supervisor process.";
        guardIntentHint.textContent = "Stop means VPN should be disconnected.";
      }
      guardPidHint.textContent = status.launch_loaded ? "Supervisor configured." : "Supervisor not configured.";
    }
    ...
    const API_TOKEN = __PANEL_API_TOKEN_JSON__;
    ...
    async function api(path, options = {}, retries = 2) {
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          const headers = Object.assign({}, options.headers || {}, { "X-MSG-Token": API_TOKEN });
    ...
    async function refresh() {
      ...
      try {
        ...
        dot.className = "dot " + (guardLocked ? "warning" : (guardMode === "launchagent" || guardMode === "scheduled-task") ? "running" : autoRunning ? "warning" : "stopped");
        statusText.textContent = guardLocked
          ? "Auto Guard paused by maintenance lock"
          : (guardMode === "launchagent" || guardMode === "scheduled-task")
            ? "Auto Guard running under supervisor"
            : autoRunning
              ? "Auto Guard running without supervisor"
              : "Auto Guard paused";
    ...
    startAutoGuardBtn.addEventListener("click", () => withBusy(startAutoGuardBtn, async () => {
      if (!confirm("Auto Guard will monitor the current relay. By default it only switches or true-tests while the computer and VPN traffic look idle, except when VPN is already disconnected. Continue?")) return;
    ...
    stopAutoGuardBtn.addEventListener("click", () => withBusy(stopAutoGuardBtn, async () => {
      if (!confirm("Stop Auto Guard means you do not want VPN protection now. It will stop the supervisor and disconnect Mullvad. Continue?")) return;
...
def render_index_html() -> str:
    return INDEX_HTML.replace("__PANEL_API_TOKEN_JSON__", json.dumps(PANEL_API_TOKEN))


def allowed_host_header(value: str) -> bool:
...
def reject_request(handler: BaseHTTPRequestHandler, status: int, message: str) -> None:
    path = urlparse(handler.path).path
    if path.startswith("/api/"):
        json_response(handler, status, {"ok": False, "error": message})
    else:
        text_response(handler, status, message, "text/plain; charset=utf-8")
...
def pid_alive(pid: Optional[int]) -> bool:
    return compat.process_alive(pid)


def pid_running(pid: Optional[int], needle: str) -> bool:
    return compat.pid_running(pid, needle)
...
def launchctl_service_info(label: str) -> Dict[str, Any]:
    if compat.IS_WINDOWS:
        task = compat.scheduled_task_status(label)
        return {
            "loaded": bool(task.get("loaded")),
...
def auto_guard_supervision() -> Dict[str, Any]:
    ...
    if compat.IS_WINDOWS:
        mode = "scheduled-task" if bool(launch.get("loaded")) and (launch_running or local_running) else "detached" if local_running else "paused"
    else:
...
def auto_guard_supervision_snapshot() -> Dict[str, Any]:
    if compat.IS_WINDOWS:
        ...
        local_pid = read_pid(AUTO_GUARD_PID_PATH)
        local_running = pid_running(local_pid, "inventory auto-guard")
        mode = "scheduled-task" if bool(launch.get("loaded")) and (bool(launch.get("running")) or local_running) else "detached" if local_running else "paused"
        return {
            "mode": mode,
            "running": bool(launch.get("running")) or local_running,
            "launch_loaded": bool(launch.get("loaded")),
            "launch_running": bool(launch.get("running")) or local_running,
            "launch_pid": local_pid if local_running else None,
            "launch_state": launch.get("state"),
            "local_running": local_running,
            "local_pid": local_pid if local_running else None,
            "error": launch.get("error"),
...
def stop_process(pid_path: Path, needle: str) -> bool:
    ...
    if not pid_running(pid, needle):
        pid_path.unlink(missing_ok=True)
        return False
    compat.terminate_process_tree(pid)
    pid_path.unlink(missing_ok=True)
    return True
...
def stop_launch_auto_guard(disconnect_vpn: bool = False) -> Dict[str, Any]:
    stopped = False
    if compat.IS_WINDOWS:
        if launchctl_service_info(LAUNCH_AUTO_GUARD_LABEL).get("loaded"):
            try:
                subprocess.run(
                    ["schtasks", "/End", "/TN", LAUNCH_AUTO_GUARD_LABEL],
                ...
                pass
            compat.delete_scheduled_task(LAUNCH_AUTO_GUARD_LABEL)
            stopped = True
        for path in [LAUNCH_AUTO_GUARD_PID_PATH, AUTO_GUARD_PID_PATH]:
            pid = read_pid(path)
...
def pkill_guard_patterns(patterns: List[str]) -> None:
    compat.kill_processes_matching(patterns)
...
def vpn_traffic_stats(status_text: Optional[str] = None) -> Dict[str, Any]:
    ...
    stats = compat.interface_counters(interface)
    if not stats.get("ok"):
        return traffic_totals_fallback(str(stats.get("error") or "Interface byte counters not found."), interface=interface)
    return cumulative_traffic_stats(
        str(stats.get("interface") or interface),
        int(stats.get("download_bytes") or 0),
        int(stats.get("upload_bytes") or 0),
    )
...
def start_monitor(settings: Dict[str, Any]) -> int:
    ...
    with WATCH_LOG_PATH.open("a", encoding="utf-8") as log:
        ...
        proc = subprocess.Popen(
            ...
            stderr=subprocess.STDOUT,
            text=True,
            **compat.background_popen_kwargs(),
        )
...
def auto_guard_args(settings: Dict[str, Any]) -> List[str]:
    ...
    if bool_setting(settings.get("allow_active_switching"), False):
        args.append("--allow-active-switching")
        args.append("--active-speed-when-passive-idle")
    else:
        args.append("--no-active-speed-when-passive-idle")
...
def write_windows_auto_guard_cmd(settings: Dict[str, Any]) -> None:
    WINDOWS_AUTO_GUARD_CMD.parent.mkdir(parents=True, exist_ok=True)
    LAUNCH_AUTO_GUARD_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    command = subprocess.list2cmdline(auto_guard_args(settings))
    content = (
        "@echo off\r\n"
        f"cd /d {subprocess.list2cmdline([str(APP_DIR)])}\r\n"
        f"{command} >> {subprocess.list2cmdline([str(LAUNCH_AUTO_GUARD_LOG_PATH)])} 2>&1\r\n"
    )
    WINDOWS_AUTO_GUARD_CMD.write_text(content, encoding="utf-8")


def start_detached_auto_guard(settings: Dict[str, Any]) -> int:
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    LAUNCH_AUTO_GUARD_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with LAUNCH_AUTO_GUARD_LOG_PATH.open("a", encoding="utf-8") as log:
        log.write(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting detached auto guard\n")
        proc = subprocess.Popen(
            auto_guard_args(settings),
            cwd=str(APP_DIR),
            env=command_env(),
            stdout=log,
            stderr=subprocess.STDOUT,
    ...
    AUTO_GUARD_PID_PATH.write_text(str(proc.pid), encoding="utf-8")
    LAUNCH_AUTO_GUARD_PID_PATH.parent.mkdir(parents=True, exist_ok=True)
    LAUNCH_AUTO_GUARD_PID_PATH.write_text(str(proc.pid), encoding="utf-8")
    return proc.pid


def launch_auto_guard_args() -> List[str]:
    if compat.IS_WINDOWS:
        return auto_guard_args({})
    try:
...
def start_auto_guard(settings: Dict[str, Any]) -> int:
    ...
    if not compat.IS_WINDOWS and status.get("mode") == "launchagent" and launch_auto_guard_args() == desired_args:
        return int(status.get("launch_pid") or read_pid(AUTO_GUARD_PID_PATH) or 0)
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    if status.get("local_running"):
        stop_process(AUTO_GUARD_PID_PATH, "inventory auto-guard")
    if compat.IS_WINDOWS:
        write_windows_auto_guard_cmd(settings)
        try:
            compat.create_logon_task(
                LAUNCH_AUTO_GUARD_LABEL,
                str(WINDOWS_AUTO_GUARD_CMD),
            )
        except Exception as exc:
            with AUTO_GUARD_LOG_PATH.open("a", encoding="utf-8") as log:
                log.write(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Task registration failed: {exc}\n")
        pid = start_detached_auto_guard(settings)
        return pid
...
def safe_manual_connect(hostname: str) -> Dict[str, Any]:
    if not re.fullmatch(r"[a-z0-9-]{3,80}", hostname or ""):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def request_allowed(self, require_token: bool = False) -> bool:
        if not allowed_host_header(self.headers.get("Host", "")):
            reject_request(self, 403, "Forbidden host header")
            return False
        if require_token:
            token = self.headers.get("X-MSG-Token", "")
            if not secrets.compare_digest(token, PANEL_API_TOKEN):
                reject_request(self, 403, "Missing or invalid API token")
                return False
        return True

    def do_GET(self) -> None:
        path = urlparse(self.path).path
        if not self.request_allowed(require_token=path.startswith("/api/") and path != "/api/ping"):
            return
        if path == "/":
            text_response(self, 200, render_index_html(), "text/html; charset=utf-8")
        elif path == "/api/ping":
    ...
    def do_POST(self) -> None:
        path = urlparse(self.path).path
        if not self.request_allowed(require_token=True):
            return
        try:
