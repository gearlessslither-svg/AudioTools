...
LAUNCH_LABELS = compat.supervisor_labels()
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "active_speed_when_passive_idle": False,
    "passive_low_speed_can_fail": False,
    "health_mode": "adaptive",
    "max_latency_ms": 2500,
    "speed_check_every_seconds": 0,
    ...
    "require_idle_for_switching": True,
...
def apply_overrides(config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if getattr(args, "active_speed_when_passive_idle", False):
        updated["active_speed_when_passive_idle"] = True
    if getattr(args, "allow_active_switching", False):
        updated["require_idle_for_switching"] = False
...
def nudge_mullvad_app() -> Optional[str]:
    """Ask the OS to start/focus Mullvad without requiring admin privileges."""
    return compat.nudge_mullvad_app()
...
def doctor_launchctl_status(label: str) -> Dict[str, Any]:
    if compat.IS_WINDOWS:
        ...
        detail = f"task state={task.get('state')}"
        if task.get("error"):
            detail = str(task["error"]).splitlines()[0] if str(task["error"]).strip() else detail
...
def vpn_interface_traffic(status_text: Optional[str] = None) -> Dict[str, Any]:
    ...
    return compat.interface_counters(interface)
...
def url_check(url: str, timeout: int) -> bool:
    ...
    try:
        ...
        if code >= 500:
            return False
        # curl produced no usable HTTP status (code 0). On Windows this also
        # happens for endpoints that close the TLS connection without a
        # close_notify (e.g. /generate_204): the Schannel backend reports it as
        # a hard error (exit 56) even though the response itself was fine. Fall
        # through to the urllib probe below, which tolerates the abrupt close.
    except FileNotFoundError:
        pass
...
def command_doctor(args: argparse.Namespace) -> int:
    ...
    if not args.skip_launchagents:
        for label in LAUNCH_LABELS:
            ...
            doctor_add(checks, f"supervisor:{label}", status, launch["detail"], **launch_extra)
...
def health_check(config: Dict[str, Any], last_speed_check_at: float) -> Tuple[HealthSample, float]:
    ...
    if mode == "adaptive":
        speed_every = max(0, int(config.get("speed_check_every_seconds", 0) or 0))
        passive_period = max(0, int(config.get("passive_probe_period_seconds", speed_every) or 0))
        passive_enabled = bool(config.get("passive_traffic_enabled", True))
        every = passive_period if passive_enabled and speed_every > 0 else speed_every
        if passive_enabled:
            ...
            if speed_due and passive_state.get("period_has_passive_success"):
                ...
                passive_state["period_started_at"] = now_mono
                passive_state["period_has_passive_success"] = False
                speed_due = bool(config.get("active_speed_when_passive_idle", False))
        else:
    ...
    while True:
        ...
        if sample.ok:
            ...
            if failures >= threshold and now - last_scan_at >= cooldown:
                ...
                if bool(config.get("require_idle_for_switching", True)) and not disconnected_failure:
                    print(
                        "Work-safe mode: VPN is still connected, so relay scan/reconnect is deferred. "
                        "Use inventory auto-guard idle maintenance or pass --allow-active-switching to permit active switching.",
                        flush=True,
                    )
                    time.sleep(interval)
                    continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_common(sub: argparse.ArgumentParser) -> None:
        ...
        sub.add_argument(
            "--active-speed-when-passive-idle",
            action="store_true",
            help="Allow an active download speed sample when adaptive passive traffic did not prove health.",
        )
        sub.add_argument(
        ...
        sub.add_argument(
            "--allow-active-switching",
            action="store_true",
            help="Allow watch/auto-guard flows to switch or true-test relays while the user or VPN traffic appears active.",
        )
    ...
    doctor_parser.add_argument("--skip-launchagents", action="store_true", help="Do not query launchctl/Task Scheduler services.")
    ...
    inventory_auto_guard.add_argument("--speed-check-every", type=int, default=0)
    ...
    inventory_auto_guard.add_argument(
        "--active-speed-when-passive-idle",
        dest="active_speed_when_passive_idle",
        action="store_true",
        default=None,
    )
    inventory_auto_guard.add_argument(
    ...
    inventory_auto_guard.add_argument(
        "--allow-active-switching",
        action="store_true",
        help="Allow auto-guard to switch/true-test relays while the user or VPN traffic appears active.",
    )
    inventory_auto_guard.add_argument(
        "--connecting-grace",
        type=int,
