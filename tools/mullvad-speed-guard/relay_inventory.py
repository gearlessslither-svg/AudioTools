...
def auto_guard(
    ...
    preferred_mbps: float = 8.0,
    require_idle_for_switching: bool = True,
    ...
    require_idle_for_switching = bool(require_idle_for_switching)
    ...
    guard_config["speed_check_every_seconds"] = max(
        0,
        int(guard_config.get("speed_check_every_seconds", 0) or 0),
    )
    ...
    switching_idle_monitor = idle_monitor
    if require_idle_for_switching and switching_idle_monitor is None:
        switching_idle_monitor = TrafficIdleMonitor(
            idle_after_seconds=idle_refresh_after_seconds,
            activity_threshold_bytes=idle_refresh_activity_threshold_bytes,
        )
    print(
        ...
        f"require_idle_for_switching={require_idle_for_switching} "
    ...
    while True:
        ...
        if require_idle_for_switching and not disconnected_failure:
            traffic_state = (
                switching_idle_monitor.observe()
                if switching_idle_monitor is not None
                else {"ok": False, "idle": False, "reason": "traffic idle monitor unavailable", "idle_for": 0.0}
            )
            user_ok, user_idle = user_idle_ready(idle_refresh_user_idle_seconds)
            idle_ready, idle_reason = maintenance_idle_ready(traffic_state, user_ok, user_idle)
            if not (idle_ready and user_ok):
                if not user_ok and user_idle is not None:
                    print(
                        f"Guard action deferred; user is active (idle {user_idle:.1f}s). {idle_reason}",
                        flush=True,
                    )
                else:
                    print(f"Guard action deferred until idle window: {idle_reason}", flush=True)
                time.sleep(interval)
                continue
...
def user_idle_seconds() -> Optional[float]:
    return compat.user_idle_seconds()
...
def cli(args: argparse.Namespace, config: Dict[str, Any]) -> int:
    ...
    if args.inventory_command == "auto-guard":
        ...
        if args.active_speed_when_passive_idle is not None:
            config["active_speed_when_passive_idle"] = bool(args.active_speed_when_passive_idle)
        config["passive_low_speed_can_fail"] = args.passive_low_speed_can_fail
        config["idle_refresh_enabled"] = not args.no_idle_refresh
        config["require_idle_for_switching"] = not args.allow_active_switching
        ...
        return auto_guard(
            ...
            preferred_mbps=args.preferred_mbps,
            require_idle_for_switching=not args.allow_active_switching,
