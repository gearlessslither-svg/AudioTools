...
TOKEN_PATH = RESULTS_DIR / "panel_api_token.txt"
...
FONT_FAMILY = "Segoe UI" if compat.IS_WINDOWS else "Helvetica"
WIDTH = 322
HEIGHT = 142
POLL_SECONDS = 1.25
...
class TrafficSampler:
    ...
    def fetch(self) -> Dict[str, Any]:
        headers = {"User-Agent": "mullvad-speed-guard-float/1.1"}
        token = panel_api_token()
        if token:
            headers["X-MSG-Token"] = token
        request = urllib.request.Request(PANEL_STATE_URL, headers=headers)
...
class FloatingTrafficWidget(tk.Tk):
    ...
    def build_ui(self) -> None:
        ...
        self.top = tk.Frame(self.outer, height=30, bd=0)
        ...
        self.status_dot.grid(row=0, column=0, padx=(10, 6), pady=(10, 10), sticky="w")
        self.status_dot.pack_propagate(False)

        self.title_label = tk.Label(self.top, text="VPN", font=(FONT_FAMILY, 10, "bold"), bd=0)
        ...
        self.state_label.grid(row=0, column=2, sticky="ew", padx=(12, 8))

        self.grip_label = tk.Label(self.top, text="--:--:--", font=(FONT_FAMILY, 9, "bold"), bd=0, anchor="e")
        self.grip_label.grid(row=0, column=3, sticky="e", padx=(0, 10))

        self.body = tk.Frame(self.outer, bd=0)
        self.body.pack(fill=tk.BOTH, expand=True, padx=14, pady=(9, 12))
        self.body.grid_columnconfigure(0, minsize=142)
        self.body.grid_columnconfigure(1, weight=1, minsize=136)
        self.body.grid_rowconfigure(0, weight=1)

        self.speed_col = tk.Frame(self.body, width=142, bd=0)
        self.speed_col.grid(row=0, column=0, sticky="nsew", padx=(0, 14))
        self.speed_col.grid_propagate(False)
        self.speed_col.grid_rowconfigure(1, weight=1)

        self.now_caption = tk.Label(self.speed_col, text="NOW", font=(FONT_FAMILY, 9, "bold"), anchor="w", bd=0)
        self.now_caption.grid(row=0, column=0, columnspan=2, sticky="w")

        self.speed_row = tk.Frame(self.speed_col, bd=0)
        self.speed_row.grid(row=1, column=0, columnspan=2, sticky="sw", pady=(8, 0))
        ...
        self.speed_unit = tk.Label(self.speed_row, text="Mbps", font=(FONT_FAMILY, 10, "bold"), bd=0)
        self.speed_unit.pack(side=tk.LEFT, pady=(15, 0))

        self.total_col = tk.Frame(self.body, bd=0)
        self.total_col.grid(row=0, column=1, sticky="nsew")
        self.total_col.grid_columnconfigure(0, weight=1)

        self.down_caption = tk.Label(self.total_col, text="DOWN", font=(FONT_FAMILY, 9, "bold"), anchor="w", bd=0)
        self.down_caption.grid(row=0, column=0, sticky="w")
        self.down_value = tk.Label(self.total_col, text="--", font=(FONT_FAMILY, 14, "bold"), anchor="w", bd=0)
        self.down_value.grid(row=1, column=0, sticky="w", pady=(0, 8))

        self.up_caption = tk.Label(self.total_col, text="UP", font=(FONT_FAMILY, 9, "bold"), anchor="w", bd=0)
        self.up_caption.grid(row=2, column=0, sticky="w")
        self.up_value = tk.Label(self.total_col, text="--", font=(FONT_FAMILY, 14, "bold"), anchor="w", bd=0)
        self.up_value.grid(row=3, column=0, sticky="w")
    ...
    def open_panel(self, event: Optional[tk.Event] = None) -> None:
        del event
        try:
            compat.open_path_or_url(PANEL_OPEN_URL)
        except Exception as exc:
