...
### Windows 快速入口 / Windows quick start

本地 Windows 适配版已加入以下入口脚本：
...
Windows 改造说明和可复用代码分析见 `WINDOWS_PORT_NOTES.md`。Windows 版使用 Task Scheduler 替代 macOS LaunchAgent，使用 `%LOCALAPPDATA%\MullvadSpeedGuard` 作为后台运行目录。

公司环境默认工作安全模式：Auto Guard 只要看到 VPN 仍是 `Connected`，就不会自动断链、切 relay 或做会切换 relay 的真测；默认也不做主动下载测速，只做状态、URL、延迟和被动流量判断。只有已经 `Disconnected`，或 `Connecting` 超过宽限时间，才会自动恢复。面板里的 `Allow active switching` 默认关闭，只有手动勾上才允许工作中主动切换/真测。
